...
`,
...

  {
    ...
    date: "2026-06-26",
    author: "Todd Morgan, Founder & CEO, CoreIdentity Development Group",
    excerpt: "In 2024, NIST finalized the first three post-quantum cryptography standards. The significance for AI governance has been largely missed — including by many platforms now marketing themselves as quantum-safe. Most have migrated one thing: data in transit. For autonomous AI governance, that is the least important of the three dimensions that matter — and the one least exposed to the actual threat.",
    content: `
<p>In 2024, NIST finalized the first three post-quantum cryptography standards: FIPS 203, FIPS 204, and FIPS 205. The significance for AI governance has been largely missed — including by many of the platforms now marketing themselves as "quantum-safe."</p>
<p>Most platforms claiming quantum readiness have migrated one thing: data in transit. They have adopted a post-quantum key encapsulation mechanism for transport encryption and declared the problem solved. But for autonomous AI governance, transport confidentiality is the least important of the three cryptographic dimensions that matter — and the one least exposed to the actual threat.</p>
<p>This is for the CISOs, CROs, and compliance leaders in regulated commercial industries — financial services, healthcare, insurance, legal — deploying autonomous AI agents into workflows where the integrity of the governance record is the compliance posture.</p>
<h2>What "Fully Quantum-Safe" Actually Requires</h2>
<p>Autonomous AI governance rests on answering three questions with cryptographic certainty. <strong>Who is the agent?</strong> Identity must be provable, not asserted. <strong>What did it do, and can that record be trusted later?</strong> The audit trail must be sealed so retroactive manipulation is detectable, throughout the entire retention lifetime of the record. <strong>What did it transmit?</strong> Data in transit must be protected.</p>
<p>Only the third question is addressed by the transport-layer migrations most platforms have completed. Identity and audit integrity — the first two — are where governance is most exposed, and they depend on a different primitive entirely: post-quantum digital signatures, not key encapsulation. A platform that encrypts data in transit but signs agent identities and audit records with classical RSA or ECDSA has solved the least urgent problem and left the most urgent one open.</p>
<h2>Why the Threat Is Present, Not Future</h2>
<p>The standard objection is that a cryptographically relevant quantum computer does not yet exist, so migration can wait. For audit and identity integrity, this fails — because of the Harvest Now, Decrypt Later attack. An adversary does not need a quantum computer today to threaten records produced today. They need only collect classically-signed records now, store them, and wait. When quantum capability matures, those records become forgeable — retroactively.</p>
<p>For regulated industries, this is the entire problem. A financial institution's AI audit trail must remain verifiable for its full regulatory retention period — often five to seven years. A healthcare system's record of an agent accessing protected health information carries the same multi-year obligation. If those records are signed today with classical cryptography, and a quantum computer emerges within that retention window, they become forgeable while still legally required to be trustworthy. The compliance record that must be trusted in 2031 is being produced today.</p>
<h2>The Regulatory Pressure Is Already Converging</h2>
<p>No single regulation yet mandates post-quantum cryptography for commercial AI governance. But the requirement is being assembled from multiple directions. Audit trail integrity requirements under EU AI Act high-risk provisions, DORA for financial services, HIPAA/HITECH for healthcare, and SOX Section 404 for financial controls all impose multi-year verifiability obligations on the records AI governance produces. None yet specify the cryptographic standard — but all assume those records stay trustworthy for their full retention period.</p>
<p>Operational resilience frameworks like DORA require institutions to demonstrate their critical systems maintain integrity against evolving threats. A known, NIST-standardized cryptographic vulnerability with a defined remediation path is exactly the kind of risk these frameworks expect institutions to address proactively. The institutions that recognize this convergence early will be positioned ahead of codification. The ones that wait for an explicit mandate will be retrofitting under deadline pressure.</p>
<h2>Why Retrofitting Is Harder Than Building Native</h2>
<p>For platforms that did not build post-quantum cryptography into their foundation, migration is not a single upgrade. It requires simultaneous refactoring of three independent layers: the identity layer (every agent credential, the certificate authority, the signing infrastructure), the audit layer (record-sealing and the integrity chain), and the transport layer (key establishment and data-in-transit protection). Executing all three at once, under a compliance deadline, is materially more expensive and higher-risk than evolving from an architecture where post-quantum primitives were native from the start.</p>
<p>This creates a real selection criterion for any institution evaluating an AI governance platform today: is post-quantum cryptography foundational to the architecture, or a transport-layer bolt-on that will require a disruptive migration when the standard tightens?</p>
<h2>What Full-Stack Implementation Looks Like</h2>
<p>Satisfying all three cryptographic dimensions requires implementing all three NIST FIPS post-quantum primitive classes concurrently: ML-DSA-65 (FIPS 204, lattice-based) for agent identity and certificate authority; SLH-DSA-128s (FIPS 205, hash-based) for audit record immutability and artifact signing; and ML-KEM-768 (FIPS 203) for transport confidentiality.</p>
<p>The combination is not redundancy. ML-DSA-65 and SLH-DSA-128s rest on independent mathematical foundations, so the exposure of any single primitive class does not collapse the integrity of the whole — cryptographic agility by design. Hardware custody is a separate requirement from algorithm choice: a signature produced by a software-extractable key inherits the weakness of that key regardless of algorithm strength. HSM-bound storage at FIPS 140-3 Level 3 is the custody standard that matches the algorithm assurance level.</p>
<p>CoreIdentity's AEG platform is implemented to satisfy all three dimensions across controlled production environments: ML-DSA-65 for agent identity with keys hardware-bound and never extractable, SLH-DSA-128s for governance and audit signing, and ML-KEM-768 for transport. This is the current operational state of the platform, available for technical review by qualified evaluators — one maturity point along the compliance spectrum, not a claim of certification equivalency.</p>
<h2>Governed Autonomy Has a Cryptographic Floor</h2>
<p>Governance without a verifiable, quantum-resistant root of trust is not governance. It is documentation that describes what should happen — without the ability to prove what did happen in a form that survives scrutiny across the years those records must remain trustworthy.</p>
<p>For any organization deploying autonomous AI into a regulated workflow, the question is not whether the cryptographic floor will be enforced. It is whether your governance infrastructure is already built above it, or whether it will need to be rebuilt when the standard catches up to the threat.</p>
<p>For organizations navigating this transition in financial services, healthcare, insurance, or other regulated verticals, we are available to discuss what full-stack post-quantum governance architecture looks like in operational practice.</p>
<p><em>Contact: <a href="mailto:tmorgan@coreidentitygroup.com">tmorgan@coreidentitygroup.com</a></em></p>
`
  },
];
